...
DO $$
BEGIN
    -- Check if assignmentSubmitted column exists
    IF NOT EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_name = 'marks' 
        AND column_name = 'assignmentSubmitted'  -- Check for camelCase
    ) AND NOT EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_name = 'marks' 
        AND column_name = 'assignment_submitted'  -- Check for snake_case
    ) THEN
        ...
        UPDATE marks 
        SET assignment_submitted = COALESCE(
            (marks_data->>'assignmentSubmitted')::boolean, 
            FALSE
        )
        WHERE marks_data IS NOT NULL;
        
        -- Drop the old JSONB column if it exists
        ALTER TABLE marks DROP COLUMN IF EXISTS marks_data;
    ...
    IF EXISTS (
        ...
        FROM information_schema.columns 
        WHERE table_name = 'marks' 
        AND column_name = 'assignmentSubmitted'  -- If it's in camelCase
    ) THEN
...
BEGIN
    ...
    RAISE NOTICE 'Created/updated update_student_marks function';
